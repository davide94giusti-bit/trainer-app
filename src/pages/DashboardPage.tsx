...
export default function DashboardPage() {
  ...
  return <Stack spacing={3}><Typography variant="h4">{t('nav.dashboard')}</Typography>{credits.error && <Alert severity="warning">Credit balance unavailable: {(credits.error as Error).message}</Alert>}<Grid container spacing={2}><Grid item xs={12} md={3}><StatCard label={t('dashboard.credits')} value={credits.data ?? '-'} /></Grid><Grid item xs={12} md={3}><StatCard label={t('dashboard.nextSession')} value={next ? formatDateTime(next.start_at) : '-'} detail={next?.status} /></Grid><Grid item xs={12} md={3}><StatCard label={t('dashboard.pendingBookings')} value={(bookings.data ?? []).filter((b: any) => b.status === 'pending').length} /></Grid><Grid item xs={12} md={3}><StatCard label={t('nav.notifications')} value={(notifications.data ?? []).filter((n: any) => n.status === 'unread').length} /></Grid></Grid><Card><CardContent><Typography variant="h6" gutterBottom>Upcoming work</Typography><Typography color="text.secondary">Use Sessions for class details, Workout Plan for assigned exercises, and Availability to request a new booking.</Typography></CardContent></Card></Stack>;
}
