...
export default function SettingsPage() {
  ...
  return <Stack spacing={2}><Typography variant="h4">{t('nav.settings')}</Typography>{mutation.error && <Alert severity="error">{(mutation.error as Error).message}</Alert>}{mutation.isSuccess && <Alert severity="success">{t('settings.saved')}</Alert>}<Card><CardContent><Stack spacing={2}><TextField label={t('settings.fullName')} value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} /><TextField label={t('settings.phone')} value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} /><TextField select label={t('settings.language')} value={form.language} onChange={e => setForm({ ...form, language: e.target.value as Language })}>{(['en','es','it'] as Language[]).map(v => <MenuItem key={v} value={v}>{v}</MenuItem>)}</TextField><TextField select label={t('settings.theme')} value={form.theme} onChange={e => setForm({ ...form, theme: e.target.value as ThemePreference })}>{(['light','dark','system'] as ThemePreference[]).map(v => <MenuItem key={v} value={v}>{t(`common.${v}`)}</MenuItem>)}</TextField><Button variant="contained" onClick={() => mutation.mutate()} disabled={mutation.isPending}>{t('common.save')}</Button></Stack></CardContent></Card></Stack>;
}
